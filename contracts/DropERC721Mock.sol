...
contract DropERC721Mock {
    // The 'encryptDecrypt' function provides a solution to encrypt or decrypt the provided data on-chain
    ...
    // It is a pure function, which means it doesn't interact with the contract's state variables,
    ...
    {
        // As presented in the comment below, this instruction initializes an uint256 variable ('length')
        ...
        assembly {
            ...
            // is called the "free memory pointer". As value, the free memory pointer can have values starting from
            ...
            // It's important to notice that even if the array length is limited to the number represented by 'length' variable value, the number of bytes
        ...
        // The following loop iterates over the provided 'data' array, reading it in chunks of 32 bytes length.
        // We can observe that 'i' variable, initialized with 0, is incremented with 32.
        ...
        for (uint256 i = 0; i < length; i += 32) {
            ...
            // a 32 bytes value stored on EVM stack and having the 'hash' variable as a reference.

            // Generate hash of the key and offset
            bytes32 hash = keccak256(abi.encodePacked(key, i));

            // Declare a 32 bytes 'chunk' variable and store it on the stack.
            ...
            // both in the for loop block scope (for the next XOR operation) and also in the scopes of the two next inline assembly blocks.
            bytes32 chunk;
            ...
            // stored on the stack, at the slot already referenced by 'chunk'.
            ...
            assembly {
                ...
                // Also, we can notice that the chunk's value is stored at the same position as the position it has been read from the 'data' array:
